  ...
  Story(props) {
    ...
    props.list.forEach(item => {
      switch(item.type) {
        ...
        case 'image' :
          var sortKey = item.sort;
          returnValue.push(<Image style={{width: Dimensions.get('window').width, height: (item.height * Dimensions.get('window').width / item.width)}} key={sortKey} source={{uri: item.uri}} />);
          break;
        default:
